...
module CCInitializers
  ...
  def self.opentelemetry(cc_config)
    if cc_config.dig(:otlp, :tracing, :enabled)
      ...
      if cc_config.dig(:otlp, :tracing, :propagation, :accept_sampling_instruction)
        sampler = OpenTelemetry::SDK::Trace::Samplers.parent_based(root: sampler)
      end
      ...
      if trace_api_url && !trace_api_url.empty? && !trace_api_token.empty?
