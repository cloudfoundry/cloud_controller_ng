  ...
  RSpec.describe ActionBuilder do
    describe 'action_type' do
      it 'knows timeout action' do
        expect(ActionBuilder.action_type(Bbs::Models::TimeoutAction.new)).to eq(:timeout_action)
      end

      it 'knows download action' do
        expect(ActionBuilder.action_type(Bbs::Models::DownloadAction.new)).to eq(:download_action)
      end

      it 'knows upload action' do
        expect(ActionBuilder.action_type(Bbs::Models::UploadAction.new)).to eq(:upload_action)
      end

      it 'knows emit progress action' do
        expect(ActionBuilder.action_type(Bbs::Models::EmitProgressAction.new)).to eq(:emit_progress_action)
      end

      it 'knows run action' do
        expect(ActionBuilder.action_type(Bbs::Models::RunAction.new)).to eq(:run_action)
      end

      it 'knows try action' do
        expect(ActionBuilder.action_type(Bbs::Models::TryAction.new)).to eq(:try_action)
      end

      it 'knows parallel action' do
        expect(ActionBuilder.action_type(Bbs::Models::ParallelAction.new)).to eq(:parallel_action)
      end

      it 'knows serial action' do
        expect(ActionBuilder.action_type(Bbs::Models::SerialAction.new)).to eq(:serial_action)
      end

      it 'knows codependent action' do
        expect(ActionBuilder.action_type(Bbs::Models::CodependentAction.new)).to eq(:codependent_action)
      end
    end
