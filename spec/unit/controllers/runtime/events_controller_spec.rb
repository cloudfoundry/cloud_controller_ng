  ...
  RSpec.describe EventsController do
    ...
    describe 'GET /v2/events' do
      before do
        ...
        @space_a = create(:space, organization: @org_a)
        @space_b = create(:space, organization: @org_b)

        @org_a.add_user(@user_a)
        @org_b.add_user(@user_b)

        @event_a = create(:event, space_guid: @space_a.guid, organization_guid: @org_a.guid)
        @event_b = create(:event, space_guid: @space_b.guid, organization_guid: @org_b.guid)

        @service_event = create(:event, space_guid: '', organization_guid: '', type: 'audit.service_broker.create')
      end

      describe 'default order' do
        it 'sorts by timestamp' do
          type = SecureRandom.uuid
          create(:event, timestamp: Time.new(1990, 1, 1).utc, type: type, actor: 'earlier')
          create(:event, timestamp: Time.new(2000, 1, 1).utc, type: type, actor: 'later')
          create(:event, timestamp: Time.new(1995, 1, 1).utc, type: type, actor: 'middle')
