...
RSpec.describe AppFeaturesController, type: :controller do
  ...
  let(:app_feature_gpu_response) { { 'name' => 'gpu', 'description' => 'Require GPU support for the app', 'enabled' => false } }
    ...
    it 'returns app features' do
      ...
      expect(parsed_body).to eq(
        'resources' => [app_feature_ssh_response, app_feature_revisions_response, app_feature_service_binding_k8s_response,
                        app_feature_file_based_vcap_services_response, app_feature_gpu_response],
  ...
  describe '#show' do
    ...
    it 'returns the gpu feature' do
      get :show, params: { app_guid: app_model.guid, name: 'gpu' }
      expect(parsed_body).to eq(app_feature_gpu_response)
    end
