    ...
    describe 'GET /internal/v5/syslog_drain_urls' do
      ...
      context 'rfc-1034-compliance: whitespace converted to hyphens' do
        ...
        it 'truncates trailing hyphens' do
          get '/internal/v5/syslog_drain_urls', '{}'
          expect(last_response).to be_successful
          expect(decoded_results.count).to eq(8)
      ...
      context 'when an app has no service binding' do
        ...
        it 'does not include that app' do
          get '/internal/v5/syslog_drain_urls', '{}'
          expect(last_response).to be_successful
          expect(decoded_results).not_to include(
            {
              'credentials' =>
                [{ 'apps' =>
                [{ 'app_id' => app_no_binding.guid }] }]
            }
          )
        end
      end
      ...
      context "when an app's bindings have no syslog_drain_url" do
        it 'does not include that app' do
          app_no_drain = ServiceBinding.make.app
          get '/internal/v5/syslog_drain_urls', '{}'
          expect(last_response).to be_successful
          expect(decoded_results.count).to eq(8)
          expect(decoded_results).not_to include(
      ...
      context 'when there are many service bindings on a single app' do
        ...
        it 'includes all of the syslog_drain_urls for that app' do
          get '/internal/v5/syslog_drain_urls', '{}'
          expect(last_response).to be_successful
          expect(decoded_results.length).to eq(50)
        end
      end
      ...
      describe 'paging' do
        ...
        it 'returns non-intersecting results when token is supplied' do
          ...
          saved_urls = []
          saved_results.each do |cred|
            saved_urls.append(cred['url'])
          end

          new_urls = []
          new_results.each do |cred|
            new_urls.append(cred['url'])
          end

          saved_urls.each do |url|
            expect(new_urls).not_to include(url)
          end
        end

        it 'eventually return entire collection, batch after batch' do
          get '/internal/v5/syslog_drain_urls', '{}'
          expect(last_response).to be_successful
          total_size = decoded_results.length

          token  = 0
          drains = []
          while drains.size < total_size
            get '/internal/v5/syslog_drain_urls', {
              'batch_size' => 2,
              'next_id' => token
            }
            ...
            drains.push(*decoded_results)
          end
          expect(drains.size).to eq(total_size)
