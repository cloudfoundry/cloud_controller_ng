  ...
  RSpec.describe SecurityGroupListMessage do
    describe 'validations' do
      ...
      describe 'globally enabled running' do
        it 'is invalid if globally enabled running is not an array' do
          message = SecurityGroupListMessage.from_params globally_enabled_running: 'not array'
          expect(message).to be_invalid
      ...
      describe 'globally enabled staging' do
        it 'is invalid if globally enabled staging is not an array' do
          message = SecurityGroupListMessage.from_params globally_enabled_staging: 'not array'
          expect(message).to be_invalid
          expect(message.errors[:globally_enabled_staging].length).to eq 1
        end

        it 'accepts and array of globally enabled staging' do
          message = SecurityGroupListMessage.from_params globally_enabled_staging: ['some-guid']
          expect(message).to be_valid
        end
      end
