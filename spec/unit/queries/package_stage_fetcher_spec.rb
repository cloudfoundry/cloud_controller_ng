  ...
  describe PackageStageFetcher do
    describe '#fetch' do
      ...
      context 'when the user is an admin' do
        ...
        it 'returns the package, app, space, and buildpack' do
          actual_package, actual_app, actual_space, actual_buildpack = package_stage_fetcher.fetch(package.guid, buildpack.guid)
        ...
        context 'when the buildpack does not exist' do
          it 'returns the package, app, space, and NIL buildpack' do
            actual_package, actual_app, actual_space, actual_buildpack = package_stage_fetcher.fetch(package.guid, 'fake-guid')
            expect(actual_package).to eq(package)
            expect(actual_app).to eq(app_model)
            expect(actual_space).to eq(space)
      ...
      context 'when the organization is not active' do
        ...
        it 'returns package, app and space to be nil' do
          actual_package, actual_app, actual_space, actual_buildpack = package_stage_fetcher.fetch(package.guid, buildpack.guid)
      ...
      context 'when the user is a space developer' do
        ...
        it 'returns the process, app and space' do
          actual_package, actual_app, actual_space, actual_buildpack = package_stage_fetcher.fetch(package.guid, buildpack.guid)
          expect(actual_package).to eq(package)
          expect(actual_app).to eq(app_model)
          expect(actual_space).to eq(space)
