    ...
    RSpec.describe PendingBuildCleanup, job_context: :worker do
      ...
      describe '#perform' do
        ...
        context 'with builds which have been staging for too long' do
          let!(:build1) { create(:build_model, state: BuildModel::STAGING_STATE) }
          let!(:build2) { create(:build_model, state: BuildModel::STAGING_STATE) }
        ...
        it 'ignores builds that have not been staging for too long' do
          build1 = create(:build_model, state: BuildModel::STAGING_STATE)
          build2 = create(:build_model, state: BuildModel::STAGING_STATE)
