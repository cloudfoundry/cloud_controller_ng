    ...
    describe PendingPackagesCleanup do
      ...
      describe '#perform' do
        context 'with packages which have been pending for too long' do
          let!(:app1) { AppFactory.make(package_pending_since: Time.now.utc - expiration_in_seconds - 1.second) }
          let!(:app2) { AppFactory.make(package_pending_since: Time.now.utc - expiration_in_seconds - 2.second) }
        ...
        it "ignores apps that haven't been pending for too long" do
          app1 = AppFactory.make(package_pending_since: Time.now.utc - expiration_in_seconds + 1.second)
          app2 = AppFactory.make(package_pending_since: Time.now.utc - expiration_in_seconds + 2.second)
