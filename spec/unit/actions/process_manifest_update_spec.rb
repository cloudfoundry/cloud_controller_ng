require 'spec_helper'
require 'actions/process_manifest_update'

module VCAP::CloudController
  RSpec.describe ProcessManifestUpdate do
    subject(:process_update) { ProcessManifestUpdate.new(user_audit_info) }
    ...
    describe '#update' do
      ...
      context 'when the process is invalid' do
        ...
        it 'raises an invalid error' do
          expect {
            process_update.update(process, message)
          }.to raise_error(ProcessManifestUpdate::InvalidProcess, 'the message')
        end
      end
    end
  end
