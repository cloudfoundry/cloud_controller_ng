...
module VCAP::CloudController
  RSpec.describe SidecarUpdate do
    ...
    let!(:process) { create(:process_model, app: app, memory: 500, type: 'other_worker') }
    ...
    describe '.update' do
      ...
      context 'when partially updating process_types' do
        ...
        context 'memory allocation' do
          let!(:new_worker_process) { create(:process_model, app: app, memory: 100, type: 'worker') }
      ...
      context 'sidecar memory allocation' do
        ...
        context 'the memory allocated to the sidecar exceeds the memory allocated for the newly associated process' do
          ...
          let!(:totes_new_process) { create(:process_model, app: app, memory: 500, type: 'totes_new') }
        ...
        context 'the memory allocated to the sidecar exceeds half the memory allocated for the newly associated process' do
          ...
          let!(:process) { create(:process_model, app: app, memory: 500, type: 'other_worker') }
          let!(:new_process) { create(:process_model, app: app, memory: 500, type: 'other_worker') }
