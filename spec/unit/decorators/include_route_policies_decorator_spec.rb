...
module VCAP::CloudController
  RSpec.describe IncludeRoutePoliciesDecorator do
    ...
    it 'decorates the given hash with route_policies from routes' do
      route_policy1 = RoutePolicy.create(route: route1, source: 'cf:app:app-guid-1')
      route_policy2 = RoutePolicy.create(route: route2, source: 'cf:app:app-guid-2')
    ...
    it 'includes multiple policies for the same route' do
      policy_a = RoutePolicy.create(route: route1, source: 'cf:app:app-guid-a')
      policy_b = RoutePolicy.create(route: route1, source: 'cf:app:app-guid-b')
      hash = subject.decorate({}, [route1])
      expect(hash[:included][:route_policies]).to contain_exactly(
