  ...
  RSpec.describe FieldServicePlanServiceBrokerDecorator do
    describe '.decorate' do
      ...
      it 'decorated the given hash with broker name and guid' do
        ...
        decorator = described_class.new({ 'service_offering.service_broker': %w[name guid] })
    ...
    describe '.match?' do
      it_behaves_like 'field decorator match?', 'service_offering.service_broker', %w[name guid]
    end
  end
end
