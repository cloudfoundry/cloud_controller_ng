...
module VCAP::CloudController
  RSpec.describe ProcessFetcher do
    ...
    describe '.fetch' do
      ...
      let(:space) { app.space }
      let!(:process) { create(:process, app:) }
    ...
    describe 'fetch_for_app_by_type' do
      ...
      context 'when there are multiple matching processes by type' do
        let!(:process_2) { create(:process, app: app, guid: 'process_2', created_at: process.created_at) }
        let!(:process_3) { create(:process, app: app, guid: 'process_3', created_at: process.created_at + 2) }
        let!(:process_4) { create(:process, app: app, guid: 'process_4', created_at: process.created_at + 3) }
        let!(:process_5) { create(:process, app: app, guid: 'process_5', created_at: process.created_at + 2) }
      ...
      context 'when multiple matching processes were created simultaneously' do
        let!(:process_2) { create(:process, app: app, guid: 'process_2', created_at: process.created_at) }
        let!(:process_3) { create(:process, app: app, guid: 'process_3', created_at: process.created_at) }
