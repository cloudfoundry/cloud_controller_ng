...
module VCAP::CloudController
  RSpec.describe AppRevisionsListFetcher do
    ...
    describe '#fetch_deployed' do
      ...
      let!(:process1) { create(:process_model, app: app, revision: revision1, type: 'web', state: 'STARTED') }
      let!(:process2) { create(:process_model, app: app, revision: revision2, type: 'web', state: 'STOPPED') }
      ...
      it 'handles processes with no revisions' do
        create(:process_model, app: app, type: 'web', state: 'STARTED')
        expect(subject).to contain_exactly(revision1)
      end
    end
  end
