  ...
  module Diego
    RSpec.describe LifecycleBundleUriGenerator do
      ...
      it 'creates a file server url for a bundle path' do
        expect(LifecycleBundleUriGenerator.uri('path/bundle.tgz')).to eq('https://file-server.example.com:1234/v1/static/path/bundle.tgz')
      end
      ...
      it 'returns back a full url' do
        expect(LifecycleBundleUriGenerator.uri('http://bundle.example.com')).to eq('http://bundle.example.com')
      end
