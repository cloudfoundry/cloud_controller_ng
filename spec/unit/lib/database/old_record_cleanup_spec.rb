...
RSpec.describe Database::OldRecordCleanup do
  describe '#delete' do
    ...
    context 'when keep_running_records is requested for a model without usage lifecycles' do
      it 'raises when the cleanup is built, rather than silently deleting the records of running resources' do
        expect do
          Database::OldRecordCleanup.new(VCAP::CloudController::Event, cutoff_age_in_days: 1, keep_running_records: true)
        end.to raise_error(ArgumentError, /usage_lifecycles/)
      end
    end
