...
module CloudController
  module Blobstore
    RSpec.describe InternalUrlGenerator do
      ...
      let(:package_blobstore) { instance_double(Blobstore::Client, blob:) }
      let(:buildpack_cache_blobstore) { instance_double(Blobstore::Client, blob:) }
      let(:admin_buildpack_blobstore) { instance_double(Blobstore::Client, blob:) }
      let(:droplet_blobstore) { instance_double(Blobstore::Client, blob:) }

      let(:internal_url) { 'http://s3.internal.example.com/signed' }
      let(:blob) { instance_double(Blobstore::Blob, internal_download_url: internal_url) }
