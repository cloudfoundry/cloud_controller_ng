    ...
    describe '#build' do
      ...
      describe 'OpenTelemetry Middlewares' do
        ...
        it 'uses OpenTelemetryMiddlewares when tracing is enabled' do
          builder.build(TestConfig.override(otel: { tracing: { enabled: true, sampling_ratio: 1.0 } }), request_metrics, request_logs).to_app
        ...
        it 'does not use OpenTelemetryMiddlewares when tracing is disabled' do
          builder.build(TestConfig.override(otel: { tracing: { enabled: false } }), request_metrics, request_logs).to_app
