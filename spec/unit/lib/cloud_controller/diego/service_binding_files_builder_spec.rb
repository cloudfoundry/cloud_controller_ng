...
module VCAP::CloudController::Diego
  ...
  RSpec.shared_examples 'mapping of binding metadata' do |name|
    it 'maps service binding metadata attributes to files' do
      expect(service_binding_files.find { |f| f.name == "#{directory}/binding-guid" }).to have_attributes(value: binding.guid)
      expect(service_binding_files.find { |f| f.name == "#{directory}/name" }).to have_attributes(value: name || 'binding-name')
      expect(service_binding_files.find { |f| f.name == "#{directory}/binding-name" }).to have_attributes(value: 'binding-name') if name.nil?
    end
  end
  ...
  RSpec.shared_examples 'mapping of instance metadata' do |instance_name|
    it 'maps service instance metadata attributes to files' do
      expect(service_binding_files.find { |f| f.name == "#{directory}/instance-guid" }).to have_attributes(value: instance.guid)
      expect(service_binding_files.find { |f| f.name == "#{directory}/instance-name" }).to have_attributes(value: instance_name || 'instance-name')
    end
  end
      ...
      describe 'mapping rules for service binding files' do
        ...
        context 'when syslog_drain_url is set' do
          ...
          it 'maps the attribute to a file' do
            expect(service_binding_files.find { |f| f.name == 'binding-name/syslog-drain-url' }).to have_attributes(value: 'https://syslog.drain')
          end
        ...
        context 'when volume_mounts is set' do
          ...
          it 'maps the attribute to a file' do
            expect(service_binding_files.find do |f|
              f.name == 'binding-name/volume-mounts'
