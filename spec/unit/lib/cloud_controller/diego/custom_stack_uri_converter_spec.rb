...
module VCAP::CloudController
  module Diego
    RSpec.describe CustomStackUriConverter do
      subject(:converter) { CustomStackUriConverter.new }

      describe '#convert' do
        it 'converts docker://host/path:tag to Diego rootfs format' do
          result = converter.convert('docker://docker.io/cloudfoundry/cflinuxfs4:1.268.0')
        ...
        it 'converts docker://host/path without tag (no fragment appended)' do
          result = converter.convert('docker://docker.io/cloudfoundry/cflinuxfs4')
          expect(result).to eq('docker://docker.io/cloudfoundry/cflinuxfs4')
        end

        it 'handles private registries' do
          result = converter.convert('docker://registry.example.com/my-org/my-stack:v2.0')
        ...
        it 'handles images without explicit registry (docker hub shorthand)' do
          result = converter.convert('docker://ubuntu:22.04')
          expect(result).to eq('docker:///library/ubuntu#22.04')
        end

        it 'handles digest references' do
          digest = 'sha256:abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890'
          result = converter.convert("docker://docker.io/cloudfoundry/cflinuxfs4@#{digest}")
