      ...
      context 'pagination options' do
        ...
        context 'when order_by has been queried, it includes order_direction prefix' do
          ...
          it 'sets the pagination options' do
            ...
            expect(first_page).to eq("/cloudfoundry/is-great?order_by=+#{order_by}&page=1&per_page=#{per_page}")
            expect(last_page).to eq("/cloudfoundry/is-great?order_by=+#{order_by}&page=3&per_page=#{per_page}")
            expect(next_page).to eq("/cloudfoundry/is-great?order_by=+#{order_by}&page=3&per_page=#{per_page}")
            expect(previous_page).to eq("/cloudfoundry/is-great?order_by=+#{order_by}&page=1&per_page=#{per_page}")
          end
