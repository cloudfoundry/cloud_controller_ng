...
module VCAP::CloudController
 describe ProcessModel, type: :model do
    describe "old app behavior" do
      ...
      describe "Validations" do
        ...
        describe "org and space quota validator policies" do
          subject(:app) { ProcessFactory.make(:space => space) }
          let(:org) { Organization.make }
        ...
        describe "buildpack" do
          it "does allow nil value" do
            expect {
              ProcessFactory.make(buildpack: nil)
            }.to_not raise_error
          end
          ...
          context "when custom buildpacks are enabled" do
            it "does allow a public url" do
              expect {
                ProcessFactory.make(buildpack: "git://user@github.com:repo")
              }.to_not raise_error
            end

            it "allows a public http url" do
              expect {
                ProcessFactory.make(buildpack: "http://example.com/foo")
              }.to_not raise_error
            end
            ...
            it "allows a buildpack name" do
              ...
              app = nil
              expect {
                app = ProcessFactory.make(buildpack: admin_buildpack.name)
              }.to_not raise_error
          ...
          context "when custom buildpacks are disabled and the buildpack attribute is being changed" do
            ...
            it "does not allow a private git url" do
              expect {
                app = ProcessFactory.make(buildpack: "git@example.com:foo.git")
              }.to raise_error(Sequel::ValidationFailed, /custom buildpacks are disabled/)
            end

            it "does not allow a private git url with ssh schema" do
              expect {
                app = ProcessFactory.make(buildpack: "ssh://git@example.com:foo.git")
              }.to raise_error(Sequel::ValidationFailed, /custom buildpacks are disabled/)
            end
          end
          ...
          context "when custom buildpacks are disabled after app creation" do
            it "permits the change even though the buildpack is still custom" do
              app = ProcessFactory.make(buildpack: "git://user@github.com:repo")
          ...
          it "does not allow a non-url string" do
            expect {
              app = ProcessFactory.make(buildpack: "Hello, world!")
        ...
        describe "name" do
          ...
          it "does not allow the same name in a different case" do
            ProcessFactory.make(:name => "lowercase", :space => space)
        ...
        describe "quota" do
          ...
          context "app update" do
            ...
            let(:space) { Space.make(:organization => org) }
            subject!(:app) { ProcessFactory.make(space: space, memory: 64, instances: 2, state: "STARTED", package_hash: "a-hash") }
      ...
      describe "#in_suspended_org?" do
        let(:space) { Space.make }
        subject(:app) { ProcessModel.new(space: space) }
      ...
      describe "#audit_hash" do
        it "should return uncensored data unchanged" do
          ...
          expect(ProcessModel.audit_hash(request_hash)).to eq(request_hash)
        end

        it "should obfuscate censored data" do
          request_hash = {"command" => "PASSWORD=foo ./start"}
          expect(ProcessModel.audit_hash(request_hash)).to eq({"command" => "PRIVATE DATA HIDDEN"})
        end
      end
        ...
        context "app is already staged" do
          ...
          context "When the droplet hash is nil" do
            it "should return nul" do
              app_without_droplet = ProcessFactory.make(droplet_hash: nil)
      ...
      describe "vcap_application" do
        it "has the expected values" do
          app = ProcessFactory.make(memory: 259, disk_quota: 799, file_descriptors: 1234, name: "app-name")
          expected_hash = {
              limits: {
                  mem: 259,
                  disk: 799,
      ...
      describe "#environment_json" do
        it "deserializes the serialized value" do
          app = ProcessFactory.make(:environment_json => {"jesse" => "awesome"})
        ...
        describe "env is encrypted" do
          ...
          let!(:app) { ProcessFactory.make(:environment_json => env) }
          let(:last_row) { VCAP::CloudController::ProcessModel.dataset.naked.order_by(:id).last }
          ...
          it "salt is unique for each app" do
            app_2 = ProcessFactory.make(:environment_json => env)
      ...
      describe "#database_uri" do
        let(:space) { Space.make }
        let(:app) { ProcessModel.make(:environment_json => {"jesse" => "awesome"}, :space => space) }
      ...
      describe "#system_env_json" do
        context "when there are no services" do
          it "contains an empty vcap_services" do
            app = ProcessModel.make(:environment_json => {"jesse" => "awesome"})
      ...
      describe "command" do
        it "stores the command in the metadata" do
          app = ProcessFactory.make(:command => "foobar")
      ...
      describe "console" do
        it "stores the command in the metadata" do
          app = ProcessFactory.make(:console => true)
        ...
        it "returns false if console was set to false" do
          app = ProcessFactory.make(:console => false)
      ...
      describe "debug" do
        it "stores the command in the metadata" do
          app = ProcessFactory.make(:debug => "suspend")
        ...
        it "returns nil if debug was explicitly set to nil" do
          app = ProcessFactory.make(:debug => nil)
        ...
        context "when staging has completed" do
          context "and the app state remains STARTED" do
            it "creates an app usage event with BUILDPACK_SET as the state" do
              app = ProcessFactory.make(package_hash: "abc", state: "STARTED", package_state: "STAGED")
              expect {
                app.update_detected_buildpack(detect_output, nil)
          ...
          context "and the app state is no longer STARTED" do
            it "does ont create an app usage event" do
              app = ProcessFactory.make(package_hash: "abc", state: "STOPPED")
              expect {
                app.update_detected_buildpack(detect_output, nil)
      ...
      describe "buildpack=" do
        ...
        context "switching between buildpacks" do
          it "allows changing from admin buildpacks to a git url" do
            buildpack = Buildpack.make
            app = ProcessModel.new(buildpack: buildpack.name)
            app.buildpack = valid_git_url
            expect(app.buildpack).to eql(CustomBuildpack.new(valid_git_url))
          end
          ...
          it "allows changing from git url to admin buildpack" do
            buildpack = Buildpack.make
            app = ProcessModel.new(buildpack: valid_git_url)
            app.buildpack = buildpack.name
            expect(app.buildpack).to eql(buildpack)
          end
        end
      ...
      describe "custom_buildpack_url" do
        context "when a custom buildpack is associated with the app" do
          it "should be the custom url" do
            app = ProcessModel.make(buildpack: "https://example.com/repo.git")
      ...
      describe "health_check_timeout" do
        ...
        context "when the health_check_timeout was not specified" do
          ...
          it "should not raise error if value is nil" do
            expect {
              ProcessFactory.make(health_check_timeout: nil)
            }.to_not raise_error
          end
        end
        ...
        context "when a valid health_check_timeout is specified" do
          it "should use that value" do
            app = ProcessFactory.make(health_check_timeout: 256)
      ...
      describe "package_hash=" do
        let(:app) { ProcessFactory.make(:package_hash => "abc", :package_state => "STAGED") }
      ...
      describe "needs_staging?" do
        subject(:app) { ProcessFactory.make }
      ...
      describe "#mark_as_failed_to_stage" do
        ...
        context "when a valid reason is specified" do
          ProcessModel::STAGING_FAILED_REASONS.each do |reason|
      ...
      describe "uris" do
        it "should return the uris on the app" do
          app = ProcessFactory.make(:space => space)
          app.add_route(route)
      ...
      describe "adding routes to unsaved apps" do
        it "should set a route by guid on a new but unsaved app" do
          app = ProcessModel.new(:name => Sham.name,
                        :space => space,
                        :stack => Stack.make)
      ...
      describe "creation" do
        it "does not create an AppUsageEvent" do
          expect {
            ProcessModel.create_from_hash(name: "awesome app", space_guid: space.guid)
        ...
        describe "default_app_memory" do
          ...
          it "uses the provided memory" do
            app = ProcessModel.create_from_hash(name: "awesome app", space_guid: space.guid, memory: 100)
            expect(app.memory).to eq(100)
          end

          it "uses the default_app_memory when none is provided" do
            app = ProcessModel.create_from_hash(name: "awesome app", space_guid: space.guid)
        ...
        describe "default disk_quota" do
          ...
          it "should use the provided quota" do
            app = ProcessModel.create_from_hash(name: "test", space_guid: space.guid, disk_quota: 256)
            expect(app.disk_quota).to eq(256)
          end

          it "should use the default quota" do
            app = ProcessModel.create_from_hash(name: "test", space_guid: space.guid)
      ...
      describe "saving" do
        ...
        context "when app state changes from STARTED to STOPPED" do
          it "creates an AppUsageEvent" do
            app = ProcessFactory.make(package_hash: "abc", state: "STARTED")
            expect {
        ...
        context "when a custom buildpack was used for staging" do
          it "creates an AppUsageEvent that contains the custom buildpack url" do
            app = ProcessFactory.make(buildpack: "https://example.com/repo.git", state: "STOPPED")
            expect {
              app.update(state: "STARTED")
            }.to change {AppUsageEvent.count}.by(1)
            event = AppUsageEvent.last
      ...
      describe "destroy" do
        let(:app) { ProcessFactory.make(package_hash: "abc", package_state: "STAGED", space: space) }
        ...
        it "creates an AppUsageEvent when the app state is STARTED" do
          app = ProcessFactory.make(package_hash: "abc", package_state: "STAGED", space: space, state: "STARTED")
          expect {
            app.destroy
          }.to change { AppUsageEvent.count }.by(1)
        ...
        it "does not create an AppUsageEvent when the app state is STOPPED" do
          app = ProcessFactory.make(package_hash: "abc", package_state: "STAGED", space: space, state: "STOPPED")
          expect {
            app.destroy
          }.not_to change { AppUsageEvent.count }
        end
        ...
        context "app state changes" do
          context "creating a stopped app" do
            it "does not generate a start event or stop event" do
              ...
              ProcessFactory.make(:state => "STOPPED")
            end
          end

          context "starting a stopped app" do
            it "generates a start event" do
              app = ProcessFactory.make(:state => "STOPPED")
          ...
          context "stopping a started app" do
            it "does not generate a start event, but generates a stop event" do
              app = ProcessFactory.make(state: "STARTED", :package_hash => "abc", :package_state => "STAGED")
              expect(AppStartEvent).not_to receive(:create_from_app)
        ...
        context "when adding and removing routes", isolation: :truncation do
          ...
          context "when modifying multiple routes at one time" do
            ...
            it "calls the app observer once when multiple routes have changed" do
              ...
              ProcessModel.db.transaction(savepoint: true) do
