...
module VCAP::CloudController
  RSpec.describe VCAP::CloudController::SpaceRole, type: :model do
    ...
    context 'when there are roles' do
      ...
      it 'contains all the roles' do
        roles = VCAP::CloudController::SpaceRole.all.each_with_object({}) do |role, obj|
          obj[role.type] = role.guid
        end

        expect(roles[VCAP::CloudController::RoleTypes::ORGANIZATION_USER]).to be_nil
        expect(roles[VCAP::CloudController::RoleTypes::ORGANIZATION_MANAGER]).to be_nil
        expect(roles[VCAP::CloudController::RoleTypes::ORGANIZATION_BILLING_MANAGER]).to be_nil
        expect(roles[VCAP::CloudController::RoleTypes::ORGANIZATION_AUDITOR]).to be_nil
