...
module VCAP::CloudController
  RSpec.describe OrganizationBillingManager, type: :model do
    ...
    it 'can be created' do
      OrganizationBillingManager.create(organization_id: organization.id, user_id: user.id)

      role_found = OrganizationBillingManager.find(organization_id: organization.id, user_id: user.id)

      expect(role_found.guid).to be_a_guid
      expect(role_found.created_at).to be_a Time
      expect(role_found.updated_at).to be_a Time
      expect(role_found.type).to eq(RoleTypes::ORGANIZATION_BILLING_MANAGER)
