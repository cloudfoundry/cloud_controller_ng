  ...
  RSpec.describe 'PackageStateCalculator' do
    describe '#calculate' do
      ...
      context 'when the package failed to upload' do
        before do
          FactoryBot.create(:package, app: parent_app, state: PackageModel::FAILED_STATE)
        end
      ...
      context 'when the package is uploaded and there is no droplet or build for the app' do
        before do
          FactoryBot.create(:package, app: parent_app, package_hash: 'hash')
        end
      ...
      context 'when the current droplet is the latest droplet' do
        before do
          package = FactoryBot.create(:package, app: parent_app, package_hash: 'hash', state: PackageModel::READY_STATE)
      ...
      context 'when the latest build failed to stage' do
        before do
          FactoryBot.create(:package, app: parent_app, package_hash: 'hash', state: PackageModel::READY_STATE)
      ...
      context 'when there is a newer package than current droplet' do
        before do
          ...
          parent_app.update(droplet: droplet)
          FactoryBot.create(:package, app: parent_app, package_hash: 'hash', state: PackageModel::READY_STATE, created_at: droplet.created_at + 10.seconds)
        end
      ...
      context 'when the latest droplet has no package but there is a previous package' do
        before do
          previous_package = FactoryBot.create(:package, app: parent_app, package_hash: 'hash', state: PackageModel::FAILED_STATE)
