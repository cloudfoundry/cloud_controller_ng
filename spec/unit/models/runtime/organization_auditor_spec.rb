...
module VCAP::CloudController
  RSpec.describe OrganizationAuditor, type: :model do
    ...
    it 'can be created' do
      OrganizationAuditor.create(organization_id: organization.id, user_id: user.id)

      role_found = OrganizationAuditor.find(organization_id: organization.id, user_id: user.id)

      expect(role_found.guid).to be_a_guid
      expect(role_found.created_at).to be_a Time
      expect(role_found.updated_at).to be_a Time
      expect(role_found.type).to eq(RoleTypes::ORGANIZATION_AUDITOR)
