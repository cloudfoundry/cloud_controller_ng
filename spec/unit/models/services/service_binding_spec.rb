...
module VCAP::CloudController
  RSpec.describe VCAP::CloudController::ServiceBinding, type: :model do
    ...
    describe 'Associations' do
      it { is_expected.to have_associated :app, associated_instance: ->(binding) { create(:app_model, space: binding.space) } }
      it { is_expected.to have_associated :service_instance, associated_instance: ->(binding) { create(:service_instance, space: binding.space) } }

      it 'has a v2 app through the v3 app' do
        service_binding = create(:service_binding)
        app = service_binding.app

        create(:process_model, app: app, type: 'non-web')
        expect(service_binding.reload.v2_app).to be_nil

        web_process = create(:process_model, app: app, type: 'web')
    ...
    describe 'Validations' do
      ...
      context 'when a binding already exists with the same app_guid and name' do
        ...
        context 'and the name is null' do
          let(:existing_binding) do
            create(:service_binding, app: app, name: nil, service_instance: service_instance, type: 'app')
          end

          it 'does NOT add a uniqueness error' do
            other_service_instance = create(:service_instance, space: existing_binding.space)
      ...
      describe 'changing the binding after creation' do
        subject(:binding) { create(:service_binding) }
        ...
        describe 'the associated service instance' do
          ...
          it 'does not allow changing service_instance after it has been set' do
            binding.service_instance = create(:service_instance, space: binding.app.space)
      ...
      describe 'service instance and app space matching' do
        ...
        context 'when the service instance and the app are in different spaces' do
          ...
          context 'when the service instance has been shared into the app space' do
            ...
            it 'is valid' do
              expect(create(:service_binding, service_instance:, app:)).to be_valid
            end
          end
        end
    ...
    describe 'logging service bindings' do
      ...
      context 'service that does not require syslog_drain' do
        ...
        it 'allows a non syslog_drain with a nil syslog drain url' do
          expect do
            service_binding = create(:service_binding, service_instance:)
      ...
      context 'service that does require a syslog_drain' do
        let(:service) { create(:service, requires: ['syslog_drain']) }
    ...
    describe 'restaging' do
      ...
      let(:service_instance) { create(:managed_service_instance, space: v2_app.space) }
    ...
    describe '#user_visibility_filter' do
      let(:app_model) { create(:app_model) }
      let!(:service_instance) { create(:managed_service_instance) }
      let!(:other_binding) { create(:service_binding) }
      let!(:service_binding) do
        service_instance.add_shared_space(app_model.space)
        create(:service_binding, service_instance: service_instance, app: app_model)
      end
    ...
    describe '#save_with_new_operation' do
      ...
      let(:app) { create(:app_model, space: service_instance.space) }
      let(:binding) do
        ServiceBinding.new(
    ...
    describe '#destroy' do
      it 'cascades deletion of related dependencies' do
        ...
        last_operation = create(:service_binding_operation)
