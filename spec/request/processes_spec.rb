...
RSpec.describe 'Processes' do
  ...
  describe 'GET /v3/processes' do
    ...
    context 'eager loading' do
      ...
      let!(:cnb_process_1) { create(:process, :cnb, app: cnb_app) }
      let!(:cnb_process_2) { create(:process, :cnb, app: cnb_app) }
  ...
  describe 'GET stats' do
    let(:process) { create(:process, :process, type: 'worker', app: app_model) }
    let(:net_info_1) do
      {
        address: '1.2.3.4',
  ...
  describe 'GET /v3/processes/:guid/process_instances' do
    let(:process) { create(:process, :process, app: app_model, state: VCAP::CloudController::ProcessModel::STARTED) }
  ...
  describe 'DELETE /v3/processes/:guid/instances/:index' do
    ...
    it 'terminates a single instance of a process' do
      process = create(:process, :process, type: 'web', app: app_model)
    ...
    context 'permissions' do
      let(:process) { create(:process, :process, type: 'web', app: app_model) }
  ...
  describe 'GET /v3/apps/:guid/processes' do
    ...
    let!(:process3) do
      create(:process, :process, app: app_model, revision: revision3)
    end
  ...
  describe 'GET /v3/apps/:guid/processes/:type' do
    ...
    it 'redacts information for auditors' do
      create(:process, :process, app: app_model, type: 'web', command: 'rackup')
