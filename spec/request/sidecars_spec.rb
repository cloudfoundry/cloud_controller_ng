...
RSpec.describe 'Sidecars' do
  ...
  describe 'POST /v3/apps/:guid/sidecars' do
    ...
    describe 'validates sidecar memory' do
      let!(:process) { create(:process_model, app: app_model, memory: 100, type: 'other_worker') }
      let(:sidecar_params) do
        {
          name: 'sidecar_one',
          command: 'bundle exec rackup',
  ...
  describe 'PATCH /v3/apps/:guid/sidecars' do
    ...
    describe 'validates sidecar memory' do
      let!(:process) { create(:process_model, app: app_model, memory: 500, type: 'other_worker') }
      let(:sidecar_params) do
        {
          name: 'sidecar_one',
          command: 'bundle exec rackup',
