...
RSpec.describe 'Roles Request' do
  ...
  describe 'POST /v3/roles' do
    ...
    context 'creating a organization role' do
      ...
      before do
        org.add_user(user_with_role)
        allow(uaa_client).to receive(:users_for_ids).with([user_with_role.guid]).and_return({ user_with_role.guid => { 'username' => user_with_role.username } })
      end
      ...
      context 'when user is invalid' do
        ...
        before do
          allow(uaa_client).to receive(:users_for_ids).with(['not-a-real-user']).and_return({})
        end
    ...
    context 'creating a role for a user that exists in UAA but not CCDB' do
      let(:expected_response) do
        {
          guid: UUID_REGEX,
          created_at: iso8601,
