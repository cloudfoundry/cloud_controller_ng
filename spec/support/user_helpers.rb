module UserHelpers
  ...
  def allow_user_secret_access(user, space:)
    allow(permissions_double(user)).to receive(:can_read_secrets_in_space?).with(space.id, space.organization_guid).and_return(true)
    allow(permissions_double(user)).to receive(:can_read_app_environment_variables?).with(space.id, space.organization_guid).and_return(true)
    allow(permissions_double(user)).to receive(:can_read_system_environment_variables?).with(space.id, space.organization_guid).and_return(true)
  end
  ...
  def allow_user_read_access_for(user, orgs: [], spaces: [])
    ...
    spaces.each do |space|
      allow(permissions_double(user)).to receive(:can_read_from_space?).with(space.id, space.organization_guid).and_return(true)
      allow(permissions_double(user)).to receive(:can_read_from_space?).with(space.id, space.organization_guid).and_return(true)
    end
  ...
  def disallow_user_read_access(user, space:)
    allow(permissions_double(user)).to receive(:can_read_from_space?).with(space.id, space.organization_guid).and_return(false)
    allow(permissions_double(user)).to receive(:can_read_from_space?).with(space.id, space.organization_guid).and_return(false)
  end
  ...
  def disallow_user_secret_access(user, space:)
    allow(permissions_double(user)).to receive(:can_read_secrets_in_space?).with(space.id, space.organization_guid).and_return(false)
    allow(permissions_double(user)).to receive(:can_read_app_environment_variables?).with(space.id, space.organization_guid).and_return(false)
    allow(permissions_double(user)).to receive(:can_read_system_environment_variables?).with(space.id, space.organization_guid).and_return(false)
  end
