RSpec::Matchers.define :have_annotations do |*expected|
  ...
  failure_message do
    if expected.any?
      "Annotations don't match: Expected #{expected} got #{actual_annotations}"
    else
      "Annotations not expected: got #{actual_annotations}"
    end
  end
  ...
  failure_message_when_negated do
    if expected.any?
      ...
      "Annotations found when not expected: got #{actual_annotations}"
    end
  end
end
