...
module TrafficController
  RSpec.describe Client do
    ...
    subject(:client) { Client.new(url: doppler_url, auth_token: auth_token) }
    ...
    describe '#container_metrics' do
      ...
      it 'returns an array of Envelopes' do
        expect(client.container_metrics(app_guid: 'example-app-guid')).to match_array([
      ...
      context 'when it does not return successfully' do
        ...
        it 'raises' do
          expect { client.container_metrics(app_guid: 'example-app-guid') }.to raise_error(ResponseError, /status: 404, body: not found/)
        end
      end
      ...
      context 'when it fails to make the request' do
        ...
        it 'raises' do
          expect { client.container_metrics(app_guid: 'example-app-guid') }.to raise_error(RequestError, /error message/)
        end
      end
      ...
      context 'when the response is not a valid multipart body' do
        ...
        it 'returns an empty array' do
          expect(client.container_metrics(app_guid: 'example-app-guid')).to be_empty
        end
      end
      ...
      context 'when the response does not contain the boundary in the "Content-Type" header' do
        ...
        it 'raises' do
          expect {
            client.container_metrics(app_guid: 'example-app-guid')
      ...
      context 'when a part cannot be decoded by ProtoBuf' do
        ...
        it 'raises' do
          expect { client.container_metrics(app_guid: 'example-app-guid') }.to raise_error(DecodeError)
        end
      end
    end
  end
