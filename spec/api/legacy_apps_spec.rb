...
module VCAP::CloudController
  describe VCAP::CloudController::LegacyApps do
    ...
    describe "GET /apps/Lname/crashes" do
      before do
        @app = Models::App.make(:space => user.default_space)
      end

      it "should delegate to v2 crashes api with path" do
        files_obj = mock("crashes")

        Crashes.should_receive(:new).once.
          and_return(files_obj)
        files_obj.should_receive(:dispatch).once.
          with(:crashes, @app.guid).and_return([HTTP::OK, "crashes"])
