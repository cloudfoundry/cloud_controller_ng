...
module VCAP::CloudController
  ...
  describe VCAP::CloudController do
    describe "signing key updates" do
...
-----END PUBLIC KEY-----
      ...
      it "uses a new signing key if it successfully decodes" do
        headers = headers_for(@user, :signing_key => @privkey2, :email => @email)

        CF::UAA::Misc.stubs(:validation_key).returns('value' => @pubkey2)
        get "/users/#{@email}", {}, headers
        last_response.status.should == 200
        #assigns(:verification_key).should == @pubkey2 # todo: verify caching
      end

      it "fails if new key cannot decode" do
        headers = headers_for(@user, :signing_key => @privkey2, :email => @email)

        CF::UAA::Misc.stubs(:validation_key).returns('value' => 'nonsense')
        get "/users/#{@email}", {}, headers
