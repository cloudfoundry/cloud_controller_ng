...
describe VCAP::CloudController::Framework do
  ...
  describe "related permissions" do
    ...
    before do
      3.times { VCAP::CloudController::Models::App.make(:app_space => @app_space_a, :framework => framework) }
      2.times { VCAP::CloudController::Models::App.make(:app_space => @app_space_b, :framework => framework) }
    end
    ...
    describe "GET /v2/frameworks/:guid?inline-relations-depth=1" do
      it "should limit inlined apps those visible to the user making the call" do
        get "/v2/frameworks/#{framework.guid}", { "inline-relations-depth" => 1 }, headers_for(@app_space_a_developer)
        last_response.should be_ok
        decoded_response["entity"]["apps"].count.should == 3

        get "/v2/frameworks/#{framework.guid}", { "inline-relations-depth" => 1 }, headers_for(@app_space_b_developer)
        last_response.should be_ok
    ...
    describe "GET /v2/frameworks/:guid/apps" do
      it "should limit inlined apps those visible to the user making the call" do
        get "/v2/frameworks/#{framework.guid}/apps", {}, headers_for(@app_space_a_developer)
        last_response.should be_ok
        decoded_response["total_results"].should == 3

        get "/v2/frameworks/#{framework.guid}/apps", {}, headers_for(@app_space_b_developer)
        last_response.should be_ok
