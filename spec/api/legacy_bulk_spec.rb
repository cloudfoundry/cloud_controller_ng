  ...
  describe LegacyBulk do
    ...
    describe ".register_subscription" do
      it "should be able to discover credentials through NATS" do
        ...
        callback = nil
        message_bus.should_receive(:subscribe) do |subject, &blk|
          subject.should == "cloudcontroller.bulk.credentials.ng"
          callback = blk
        end
        LegacyBulk.register_subscription
        message_bus.should_receive(:publish) do |subject, msg|
          subject.should == "inbox"
          Yajl::Parser.parse(msg).should == {
            "user"      => @bulk_user,
            "password"  => @bulk_password,
          }
        end
        callback.call("xxx", "inbox")
      end
    end
    ...
    describe "GET", "/bulk/apps" do
      ...
      describe "with authentication" do
        ...
        it "returns non-intersecting results when token is supplied" do
          ...
          get "/bulk/apps", {
            "batch_size" => 2,
            "bulk_token" => decoded_response["bulk_token"],
          }
