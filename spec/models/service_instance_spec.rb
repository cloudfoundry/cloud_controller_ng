    ...
    context "quota" do
      ...
      context "exceed quota" do
        it "should raise error when quota is exceeded" do
          ...
          Models::ServiceInstance.make(:space => space).save(:validate => false)
          space.refresh
          expect do
            Models::ServiceInstance.make(:space => space,
                                         :service_plan => free_plan)
          end.to raise_error(Models::Organization::QuotaExceeded,
                             /#{paid_quota.total_services}/)
        end
      ...
      context "create paid services" do
        it "should raise error when created in free quota" do
          ...
          expect do
            Models::ServiceInstance.make(:space => space,
                                         :service_plan => paid_plan).save
          end.to raise_error(Models::Organization::ServicePlanNotAllowed)
        end
