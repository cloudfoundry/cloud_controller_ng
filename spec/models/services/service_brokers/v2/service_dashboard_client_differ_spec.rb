    ...
    describe '.create_changeset' do
      ...
      context 'when a claimed client is removed from the catalog' do
        ...
        it 'returns a delete command for the existing client' do
          expect(changeset).to have(1).items
          expect(changeset.first).to be_a DeleteClientCommand
      ...
      context 'when the catalog requests to create a new client, update an existing one, and delete an old one' do
        ...
        let(:service_with_client_to_be_removed) do
          CatalogService.new(service_broker, {'dashboard_client' => nil })
        end
        ...
        it 'succeeds to create all the necessary commands' do
          ...
          expect(changeset[0]).to be_a CreateClientCommand
          expect(changeset[1]).to be_a UpdateClientCommand
          expect(changeset[2]).to be_a CreateClientCommand
          expect(changeset[3]).to be_a UpdateClientCommand
          expect(changeset[4]).to be_a DeleteClientCommand
          expect(changeset[5]).to be_a DeleteClientCommand
        end
      end
    end
  end
