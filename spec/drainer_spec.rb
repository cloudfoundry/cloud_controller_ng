...
describe VCAP::CloudController::Drainer do
  ...
  describe "ready?" do
    it "should return true when ready" do
      @klass.setup
      @klass.ready?.should be_true
    end

    it "should return false when not ready" do
      @klass.setup
      @klass.drain
      @klass.ready?.should be_false
    end
  end
  ...
  context "raise errors" do
    it "should raise error during invalid decrement operation" do
      expect {
        @klass.setup
        @klass.decrement_requests
      }.to raise_error(DrainerError, "Invalid decrement operation.")
