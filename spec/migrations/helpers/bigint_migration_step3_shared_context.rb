...
RSpec.shared_context 'bigint migration step3a' do
  ...
  let(:migration_config) do
    instance_double(VCAP::CloudController::Config,
                    get: nil).tap do |cfg|
      allow(cfg).to receive(:get).with(:skip_bigint_id_migration).and_return(skip_bigint_id_migration)
      allow(cfg).to receive(:get).with(:migration_psql_concurrent_statement_timeout_in_seconds).and_return(300)
    end
  end

  before do
    allow(VCAP::CloudController::Config).to receive(:config).and_return(migration_config)
  end
...
RSpec.shared_context 'bigint migration step3b' do
  ...
  describe 'up' do
    ...
    it 'completes bigint migration: drops constraints/trigger, renames columns, maintains primary key; identity sequence is correct' do
      ...
      # Timestamp index exists before migration if applicable
