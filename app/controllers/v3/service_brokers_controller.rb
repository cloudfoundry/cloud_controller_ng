...
class ServiceBrokersController < ApplicationController
  ...
  def show
    ...
    if service_broker.space_guid
      ...
      broker_not_found! unless space && permission_queryer.can_read_services_in_space?(space.id, space.organization_guid)
    else
  ...
  def update
    ...
    if service_broker.space_guid
      space = service_broker.space
      broker_not_found! unless space && permission_queryer.can_read_from_space?(space.id, space.organization_guid)
      unauthorized! unless permission_queryer.can_write_space_scoped_service_broker?(space.id)
      suspended! unless permission_queryer.is_space_active?(space.guid)
    else
      broker_not_found! unless permission_queryer.can_read_globally?
  ...
  def destroy
    ...
    if service_broker.space.nil?
      ...
      broker_not_found! unless permission_queryer.can_read_from_space?(service_broker.space.id, service_broker.space.organization.guid)
