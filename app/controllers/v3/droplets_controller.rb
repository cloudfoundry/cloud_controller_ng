...
class DropletsController < ApplicationController
  ...
  def show
    ...
    droplet_not_found! unless droplet && permission_queryer.can_read_from_space?(droplet.space.id, droplet.space.organization.guid)
    show_secrets = permission_queryer.can_read_secrets_in_space?(droplet.space.id, droplet.space.organization.guid)
  ...
  def destroy
    ...
    droplet_not_found! unless droplet && permission_queryer.can_read_from_space?(space.id, org.guid)
  ...
  def create_copy
    ...
    droplet_not_found! unless source_droplet && permission_queryer.can_read_from_space?(source_droplet.space.id, source_droplet.space.organization.guid)

    destination_app = AppModel.where(guid: message.app_guid).first
    app_not_found! unless destination_app && permission_queryer.can_read_from_space?(destination_app.space.id, destination_app.organization.guid)
  ...
  def create_fresh
    ...
    unprocessable_app!(message.relationships_message.app_guid) unless app && permission_queryer.can_read_from_space?(app.space.id, app.organization.guid)
  ...
  def upload
    ...
    suspended! unless permission_queryer.is_space_active?(droplet.space.guid)
  ...
  def download
    ...
    unauthorized! unless permission_queryer.can_download_droplet?(droplet.space.id, droplet.space.organization.guid)
