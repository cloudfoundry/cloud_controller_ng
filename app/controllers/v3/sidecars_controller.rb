...
class SidecarsController < ApplicationController
  ...
  def index_by_app
    ...
    resource_not_found!(:app) unless app

    space = app.space
    resource_not_found!(:app) unless permission_queryer.can_read_from_space?(space.id, space.organization_id)
  ...
  def index_by_process
    ...
    resource_not_found!(:process) unless process

    space = process.space
    resource_not_found!(:process) unless permission_queryer.can_read_from_space?(space.id, space.organization_id)
    render status: :ok, json: Presenters::V3::PaginatedListPresenter.new(
      presenter: Presenters::V3::SidecarPresenter,
      paginated_result: SequelPaginator.new.get_page(dataset, message.try(:pagination_options)),
      path: base_url(resource: 'sidecars'),
