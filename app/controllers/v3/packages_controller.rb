...
class PackagesController < ApplicationController
  ...
  def create_fresh
    ...
    space = app.space
    unprocessable_app! unless permission_queryer.can_read_from_space?(space.id, space.organization_id)
    unauthorized! unless permission_queryer.can_write_to_active_space?(space.id)
    suspended! unless permission_queryer.is_space_active?(space.id)
  ...
  def create_copy
    ...
    unprocessable_app! unless permission_queryer.can_read_from_space?(destination_app_space.id, destination_app_space.organization_id)
    ...
    source_package_space = source_package.space
    unprocessable_source_package! unless permission_queryer.can_read_from_space?(source_package_space.id, source_package_space.organization_id)
    unauthorized! unless permission_queryer.can_write_to_active_space?(source_package_space.id)
    suspended! unless permission_queryer.is_space_active?(source_package_space.id)
