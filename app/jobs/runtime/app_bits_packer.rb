    ...
    module Runtime
      class AppBitsPacker < Struct.new(:app_guid, :uploaded_compressed_path, :fingerprints)
        ...
        def perform
          ...
          app_bits_package = AppBitsPackage.new(package_blobstore, global_app_bits_cache, max_package_size, VCAP::CloudController::Config.config[:directories][:tmpdir])
          app_bits_package.create(app, uploaded_compressed_path, CloudController::Blobstore::FingerprintsCollection.new(fingerprints))
        end
