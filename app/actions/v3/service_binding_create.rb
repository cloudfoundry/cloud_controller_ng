...
module VCAP::CloudController
  module V3
    class ServiceBindingCreate
      ...
      def initialize(binding_guid: nil, user_audit_info:, audit_hash:)
        @user_audit_info = user_audit_info
        @audit_hash = audit_hash
        @binding_guid = binding_guid
      end

      def bind(service_instance, app: nil, name: nil, parameters: {}, volume_mount_services_enabled: false, accepts_incomplete: false)
        binding = if @binding_guid.nil?
                    create_record(service_instance, app: app, name: name, volume_mount_services_enabled: volume_mount_services_enabled)
                  else
                    ServiceBinding.first(guid: @binding_guid)
                  end

        return binding.guid, true if service_instance.managed_instance? && @binding_guid.nil?
        ...
        begin
          ...
          return binding.guid, binding.operation_in_progress?
        rescue => e
      ...
      def create_record(service_instance, app: nil, name: nil, volume_mount_services_enabled: false)
