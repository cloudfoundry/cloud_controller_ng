...
def rename_foreign_key_internal(db, alter_table, table, current_name, new_name, &block)
  db.foreign_key_list(table).each { |fk|
...
def rename_index_internal(db, alter_table, table, columns, opts = {})
  ...
  db.indexes(table).each { | name, index |
...
def rename_permission_table_index(name, short_name, permission)
  name = name.to_s
  join_table = "#{name.pluralize}_#{permission}".to_sym
  idx_name = "#{short_name}_#{permission}_index".to_sym
  id_attr = "#{name}_id".to_sym
  rename_index(join_table, [id_attr, :user_id], :unique => true, :name => idx_name)
end
  ...
  up do
    if !self.class.name.match /oracle/i
      ...
      rename_foreign_key(:service_plans, :fk_service_plans_service_id, :fk_service_plans_service_id) do | db, alter_table |
        rename_index_internal(db, alter_table, :service_plans, [:service_id, :name], :unique => true, :case_insensitive => [:name], :name => :svc_plan_svc_id_name_index)
      end
      rename_foreign_key(:spaces, :fk_spaces_organization_id, :fk_spaces_organization_id) do | db, alter_table |
        rename_index_internal(db, alter_table, :spaces, [:organization_id, :name], :unique => true, :case_insensitive => [:name], :name => :spaces_org_id_name_index)
      end
      ...
      rename_foreign_key(:service_instances, :service_instances_space_id, :service_instances_space_id) do | db, alter_table |
        rename_index_internal(db, alter_table, :service_instances, [:space_id, :name], :unique => true, :case_insensitive => [:name], :name => :si_space_id_name_index)
      end
      ...
      rename_index(:quota_definitions, :name, :unique => true, :case_insensitive => true, :name => :qd_name_index)
      rename_common_indexes(:quota_definitions, :qd)
      rename_index(:service_auth_tokens, [:label, :provider], :unique => true, :case_insensitive => true, :name => :sat_label_provider_index)
      rename_common_indexes(:service_auth_tokens, :sat)
      rename_index(:services, [:label, :provider], :case_insensitive => true, :unique => true, :name => :services_label_provider_index)
      rename_index(:organizations, :name, :unique => true, :case_insensitive => true, :name => :organizations_name_index)
      rename_index(:routes, [:host, :domain_id], :unique => true, :case_insensitive => [:host], :name => :routes_host_domain_id_index)
      rename_common_indexes(:service_instances, :si)
      rename_index(:service_instances, :name, :case_insensitive => true, :name => :service_instances_name_index)
      rename_common_indexes(:service_bindings, :sb)
      [:users, :managers, :billing_managers, :auditors].each do |perm|
        rename_permission_table_index(:organization, :org, perm)
      end
