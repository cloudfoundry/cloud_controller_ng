    ...
    (annotation_tables + label_tables).each do |table|
      transaction do
        ...
        begin
          ...
          warn 'Cannot guarantee consistent migration for table ' \
               "#{table} as your used user lacks the \"LOCK TABLE\" Permission " \
               'or the database does not support locking tables (e.g. percona xtradb cluster).'
          warn "Continuing to do the migration for table #{table}, " \
               'there is a small chance of a migration failure due to lack of above feature ' \
               'but eventually reruns of this migration should succeed.'
        end
