...
module VCAP::WasRunningBackfill
  ...
  class << self
    ...
    #
    # One case the repair leaves alone on purpose: the resource stopped before
    # the seed wrote its baseline (so the STOPPED has a lower id), and then
    # started again before the repair ran. The repair sees a running resource
    # and adds nothing. The baseline stays, and consumers count the new run
    # from the baseline's time instead of the real STARTED -- a small overbill
    # by that gap. The cleanup keeps the baseline as the first beginning of
    # that run for as long as the resource runs. Bounded and harmless, so we
