      ...
      begin
        service_broker.db.transaction do
          ...
          uaa_changeset.each do |uaa_cmd|
            ...
            when 'add'
              @services_event_repository.create_service_dashboard_client_event('audit.service_dashboard_client.create', uaa_cmd.client_attrs, service_broker)
            when 'delete'
              @services_event_repository.create_service_dashboard_client_event('audit.service_dashboard_client.delete', uaa_cmd.client_attrs, service_broker)
            end
          end
        end
      rescue VCAP::Services::SSO::UAA::UaaError => e
