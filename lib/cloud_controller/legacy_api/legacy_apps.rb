  ...
  class LegacyApps
    ...
    def self.setup_routes
      ...
      controller.get "/apps" do
        klass.new(@config, logger, @user, request).enumerate
      end

      controller.get "/apps/:name" do |name|
        klass.new(@config, logger, @user, request).read(name)
      end

      controller.put "/apps/:name" do |name|
        klass.new(@config, logger, @user, request).update(name)
      end

      controller.delete "/apps/:name" do |name|
        klass.new(@config, logger, @user, request).delete(name)
      end

      controller.post "/apps" do
        klass.new(@config, logger, @user, request).create
      end

      controller.post "/apps/:name/application" do
        klass.new(@config, logger, @user, request).upload_resources
      end
    end
