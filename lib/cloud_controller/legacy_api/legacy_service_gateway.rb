...
module VCAP::CloudController
  class LegacyServiceGateway < LegacyApiBase
    ...
    def initialize(config, logger, request, service_auth_token)
    ...
    def self.setup_routes
      ...
      controller.get "/services/v1/offerings/:label/handles" do
        LegacyServiceGateway.new(@config, logger, request, @service_auth_token).list_handles(params[:label], DEFAULT_PROVIDER)
      end

      controller.get "/services/v1/offerings/:label/:provider/handles" do
        LegacyServiceGateway.new(@config, logger, request, @service_auth_token).list_handles(params[:label], params[:provider])
      end

      controller.post "/services/v1/offerings" do
        LegacyServiceGateway.new(@config, logger, request, @service_auth_token).create_offering
      end
    end
