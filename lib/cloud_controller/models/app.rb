...
module VCAP::CloudController::Models
  class App < Sequel::Model
    ...
    def generate_start_event?
      ...
      # once during a state change. Alsi, if the app is not in started state
      # and/or is new, then the changes to the footprint shouldn't trigger a
      # billing event.
      return true if started? && ((column_changed?(:state)) ||
        (!new? && footprint_changed?))
      false
    end
    ...
    def generate_stop_event?
      ...
      return true if !new? && ((column_changed?(:state) && stopped?) ||
