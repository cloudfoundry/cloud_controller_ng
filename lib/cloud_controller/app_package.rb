  ...
  module AppPackage
    class << self
      ...
      def to_zip(guid, uploaded_file, resources)
        ...
        logger.debug2("package_path(guid) is %s, exists? = %s" % [package_path(guid), File.exists?(package_path(guid))])
        logger.debug2("dirname(package_path(guid)) is %s, exists? = %s" % [File.dirname(package_path(guid)), File.exists?(File.dirname(package_path(guid)))])
