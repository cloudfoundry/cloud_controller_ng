    ...
    module HM9000
      class Respondent
        ...
        def process_hm9000_start(message)
          ...
          if should_start
            # Change to use AppStarterTask
            AppStarterTask.new(app, CloudController::DependencyLocator.instance.blobstore_url_generator, dea_client.config[:cc_partition]).start(specific_instances: instance_index)
